...
class Processor:
    ...
    def get_HSI_patches_rw(self, x, gt, ksize, stride=(1, 1), padding='reflect', is_indix=False, is_labeled=True):
        ...
        new_height = np.ceil(x.shape[0] / stride[0])  # 145
        new_width = np.ceil(x.shape[1] / stride[1])  # 145
        pad_needed_height = (new_height - 1) * stride[0] + ksize[0] - x.shape[0]  # 23
        pad_needed_width = (new_width - 1) * stride[1] + ksize[1] - x.shape[1]  # 23
        pad_top = int(pad_needed_height / 2)
        pad_down = int(pad_needed_height - pad_top)
        pad_left = int(pad_needed_width / 2)
        pad_right = int(pad_needed_width - pad_left)
        ...
        x_patches = rw(x, ksizes_, axes=(1, 0))  # [145, 145, 200, 24, 24]  # divide data into 5x5 blocks
        y_patches = rw(y, ksizes_, axes=(1, 0))  # [145, 145, 24, 24]
        i_1, i_2 = int((ksize[0] - 1) // 2), int((ksize[0] - 1) // 2)
        if not is_labeled:
            x_patches = x_patches.reshape((n_row_init * n_col_init, n_band, ksize[0], ksize[1]))  # [21025, 200, 24, 24]
